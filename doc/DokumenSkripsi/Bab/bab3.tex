...
\begin{itemize}
    ...
    \end{itemize}
\end{itemize}
\subsubsection{RouteResult}
Merupakan sebuah \textit{inner class} yang menyimpan hasil akhir dalam format konfigurasi sebagai string \texttt{trackInConfFormat}, yang dapat diambil dengan \textit{method} \texttt{getTrackInConfFormat()}.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \\ \textit{Method} ini menghitung jarak antara objek LatLon saat ini dengan objek LatLon lain (target).
    \end{itemize}
\end{itemize}

\subsection{UnrolledLinkedList}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{size()}
        \\ Menghitung total elemen di seluruh \texttt{UnrolledLinkedList}, termasuk yang ada di \texttt{nextList}.
        \item \texttt{cleanUpMemory()}
        \\Memangkas ukuran \texttt{internalArray} ke jumlah elemen yang sebenarnya untuk menghemat memori.
    \end{itemize}
\end{itemize}

\subsubsection{FastLinkedListIterator}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{hasNext()}
        \\ Mengecek apakah masih ada elemen berikutnya di dalam \texttt{internalArray} atau di \texttt{nextList}.
        \item \texttt{next()}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \\ Metode ini mengembalikan nilai \textit{node} yang dituju oleh \textit{edge} ini. Berguna untuk mendapatkan informasi tentang \textit{node} tujuan.
        \item \texttt{double getWeight()}
        \\ Metode ini mengembalikan bobot dari \textit{edge} saat ini, yang dapat digunakan dalam perhitungan atau penelusuran jalur dalam graf.
    \end{itemize}
\end{itemize}
...
Merupakan sebuah \textit{inner class} yang menyimpan data untuk setiap node, seperti \texttt{baseIndex}, \texttt{heapIndex}, \texttt{distance}, dan \texttt{parent}. Selain itu, diimplementasikan juga method \texttt{toString()} yang mengembalikan nilai string dari data-data tersebut yang telah diformat.