...
public class Dijkstra extends ShorestPathStrategy {
	 ...
	 * is run, {@link Dijkstra#getDistance(GraphNode)} will retrieve distance of
	 * each node from start
	 * node and {@link Dijkstra#getParent(GraphNode)} will retrieve the parent of
	 * each node.
	 * Complexity: O(|E| + |V| log |V|)
	 * 
	 * @param trackTypeIdBlacklist Set of track type ids blacklisted on navigation,
	 *                             or null to allow everything
	 * @return the distance from source, or {@link Double#POSITIVE_INFINITY} if no
	 *         path was found.
	 */
	@Override
		...
		do {
			...
			for (GraphEdge edge : edges) {
				...
				if (currentNode.distance + weight < nodeInfoLinks[edge.node].distance
						&& (trackTypeIdBlacklist == null || graph.get(edge.node).track == null
	...
	public double calculateWeight(NodeInfo currentNode, GraphEdge edge) {
		...
		// MASIH DI ANGKOT
		else if (track1.getTrackId().equals(track2.getTrackId())) {
